...
def melt_curve_deltaG(T,dH,Tm,F_n,F_u):
    """ Two state fluorescence equation to dH and Tm """

    T = T + CtoK
    Tm = Tm + CtoK
    dG = dH * (1-(T/Tm))
    F = F_n / (1 + np.exp(dG/(R*T)) + F_u / (1 + np.exp((-dG/(R*T)))))
    return F

def fit_melt_curve_no_sigma(T,p):
    """ Fits melting curve without a sigma """
    return optimize.curve_fit(melt_curve_deltaG, T, p, p0=[-5, 30,0,1])


def get_dG(T,dH,Tm):
    """Outputs dG given a T, dH and Tm"""
    T = T + CtoK
    Tm = Tm + CtoK
    return dH * (1-T/Tm)
